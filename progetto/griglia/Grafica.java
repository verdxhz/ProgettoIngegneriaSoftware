package so.ing.progetto.griglia;
...
public class Grafica extends JFrame
{
    ...
    private void secondaPagina()
    {
        ...
        p.setVisible(true);
        String immschema = mediatore.schemaIm();
        ImageIcon schema = new ImageIcon(immschema);
        JLabel immagine = new JLabel();
        immagine.setIcon(schema);
        immagine.setBounds(100, 150, 300, 300);
    ...
    private void terzaPagina()
    {
        ...
        for(int i=0;i<mediatore.getDimensione();i++)
        {
            for (int j = 0; j < mediatore.getDimensione(); j++)
            {
                ...
                casella.setBounds(150-(mediatore.getDimensione()*2)+j*70,150-(mediatore.getDimensione()*2)+i*70,70,70);
        ...
        for (Blocco b : mediatore.getBlocchi()) {
            ...
            vincolo.setBounds(casella.getX()+2,casella.getY()-26, casella.getWidth(), casella.getHeight());
            p.add(vincolo,JLayeredPane.PALETTE_LAYER);
            for(Casella c: b.caselleBlocco ){
                try {
                    griglia[c.i][c.j].setBackground(new Color(127+ i, 127+ i, 127 + i));
                }catch (IllegalArgumentException e){
                    i=0;
                    griglia[c.i][c.j].setBackground(new Color(127, 127, 127));
                }
            } i+=32;}
